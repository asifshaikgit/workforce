const getLedgerDashboardAnalytics = async (tenantDb) => {
    try {
        await tenantDb.raw(
            `DROP FUNCTION IF EXISTS GetLedgerDashboardAnalytics(
                entity_type_filter TEXT,
                payment_entity_type_filter TEXT
            );
            CREATE OR REPLACE FUNCTION GetLedgerDashboardAnalytics(
                entity_type_filter TEXT,
                payment_entity_type_filter TEXT
            )
            RETURNS TABLE (
            ...
            BEGIN 
                ...
                WITH 
                    ...
                    overdue_invoices AS (
                        ...
                        AND status IN ('Approved', 'Partially Paid')
                        AND entity_type = entity_type_filter
                    ),
                    amount_raised AS (
                        ...
                        AND status IN ('Approved', 'Paid', 'Partially Paid')
                    ),
                    amount_overdue AS (
...
module.exports = getLedgerDashboardAnalytics;
