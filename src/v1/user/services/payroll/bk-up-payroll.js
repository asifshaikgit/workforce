...
const generatePayroll = async (body) => {
    ...
    try {
        ...
        for (const item in employee) {
            ...
            let allTimesheetAreValid = false; // A variable to flag the timesheet approvals are done or not for that employee 
            ...
            if (placementExistsData.status) {
                ...
                const tsFetchConditions = { 'timesheet_hours.payroll_raised': false, 'pla.employee_id': employee[item].id, 'global_search': `"timesheet_hours"."date" >= '${moment(payroll.from_date).format('YYYY-MM-DD')}' and "timesheet_hours"."date" <= '${moment(payroll.to_date).format('YYYY-MM-DD')}' and "ts"."status" NOT IN ('Approved')` }
                ...
                if (!checkAllTsApproved.status) {
                    ...
                    if (placementData.status) {
                        ...
                        let total_worked_payroll_hours = 0;
                        let total_amount = 0;
                        let total_payroll_amount = 0;
                        let totalOtHours = 0;
                        let payrollOtPayrate = 0;
                        let totalBillableHours = 0;
                        // let payrollPayableRate = 0;
                            ...
                            if (current_hours > 0) {
                                ...
                                if (payType === 2) {  //  2 for Hourly
                                    ...
                                    timesheets_array.forEach(ts => {
                                        ...
                                        let tempPayrate = (dailyP.totalBillablePayRate ? parseFloat(dailyP.totalBillablePayRate) : 0);
                                        let tempOtPayrate = (dailyP.totalOtPayrate ? parseFloat(dailyP.totalOtPayrate) : 0);

                                        // if (payrollPayableRate < tempPayrate) {
                                        //     payrollPayableRate = tempPayrate
                                        // }
                                        if (payrollOtPayrate < tempOtPayrate) {
                                            payrollOtPayrate = tempOtPayrate
                                        }
                                ...
                                // for salary pay 
                                    ...
                                    var dailyPay = { totalHours: totalHours, totalPay: temp_payroll }
                                }
                                ...
                                if (dailyPay.PayRateArray) { // Hours 
                                    ...
                                    hoursPayRateObject = JSON.stringify([{ "hours": dailyPay.totalHours, "amount_payable": dailyPay.totalPay.toFixed(2), "pay_in": "Salary", "pay_rate": "" }])
                                }
                        ...
                        */
                        ...
                        if (payrollPaymentExists.status) {
                            let newPayrollPaymentData = {
                                total_amount: total_payroll_amount, // Total amoun need to pay for this payroll
                            ...
                            let newPayrollPaymentData = {
                                ...
                                worked_hours: total_worked_hours.toFixed(2), // Including the billable  + ot
                                total_amount: total_payroll_amount, // Total amoun need to pay for this payroll
                                balance_amount: total_payroll_amount,  // default setting the total_amount as billable amount
                                created_by: body.created_by,
                                created_at: new Date(),
                            }
                    ...
                    /**
                     * If placement exists (project is completed and not within date range), we handle their payroll & payroll_payment_details as follows:
                     * 1. Check for an existing payroll entry. If found, update it with 'updated_by' and 'updated_at' details.
                     * 2. If no existing entry is found, create a new payroll entry.
                 ...
                 */
                    ...
                    let placementsFetchCondition = { 'placements.employee_id': employee[item].id }
                    var placementData = await indexRepository.find('placements', placementDataFields, placementsFetchCondition, null, placementDataJoins)
                    /* Fetching the Employees Placement Information */
                    ...
                    if (placementData.status) {
                        ...
                        */
                        ...
                        if (payrollPaymentExists.status) {
                            let newPayrollPaymentData = {
                                total_amount: total_amount.toFixed(2),
                                balance_amount: total_amount.toFixed(2),
                                worked_hours: total_worked_hours.toFixed(2),
            ...
            /**
             * If the employee doesn't have any placements, we handle their payroll & payroll_payment_details as follows:
             * 1. Check for an existing payroll entry. If found, update it with 'updated_by' and 'updated_at' details.
             * 2. If existing entry is not found, create a new payroll entry.
             * 3. Update / create payroll payment details. If an existing entry is found, update it. If not, store payroll payment details.
             * 4. Calculate then store / update the worked_hours and total_amount as 0 for the employee's payroll_payment_details.
             */
            else {
        ...
        // // If All Timesheets are Approved
        // if (allTimesheetAreValid == 1) {
        //     /* Update the Payroll generate information */
        //     let updatePayGeneratecondition = { id: body.payroll_configuration_id };
        //     await transactionRepository.update(trx, 'payroll_configuration', updatePayGeneratecondition, { status: 'Submitted' });
        //     /* Update the Payroll generate information */
        // }
...
const payrollPlacementInfo = async (condition) => {
    ...
    const fields = ['payroll.*', 'companies.name as client_name', 'companies.logo_document_url', 'employee.display_name as employee_name', 'placements.end_date as placement_end_date', 'payroll_configuration.to_date as payroll_to_date']
    const joins = [
            ...
            for (let keyss in item.hours_rate_information) {
                ...
                for (let paykey in temp2) {
                    let temp1 = {
                        ...
                        bill_rate: temp2[paykey].bill_rate ? temp2[paykey].bill_rate : '-',
                        ot_bill_rate: temp2[paykey].ot_bill_rate ? temp2[paykey].ot_bill_rate : '-',
                        hours: temp2[paykey].hours ? temp2[paykey].hours : '-',
                        pay_type: temp2[paykey].pay_in ? temp2[paykey].pay_in : '-',
                        pay_rate: temp2[paykey].pay_rate ? parseFloat(temp2[paykey].pay_rate).toFixed(2) : '-',
                        amount_payable: temp2[paykey].amount_payable ? temp2[paykey].amount_payable : '-',
                        ot_hours: temp2[paykey].ot_hours ? temp2[paykey].ot_hours : '-',
                        ot_pay_rate: temp2[paykey].ot_pay_rate ? temp2[paykey].ot_pay_rate : '-',
                        ot_amount_payable: temp2[paykey].ot_amount_payable ? temp2[paykey].ot_amount_payable : '-',
                        total_hours: temp2[paykey].total_hours ? temp2[paykey].total_hours : '-',
                        total_amount_payable: temp2[paykey].total_amount_payable ? temp2[paykey].total_amount_payable : '-',
                    }
                    if (temp1.pay_type != '-') {
                        /**
...
const paymentsListing = async (condition, page, limit) => {
    ...
    if (payrollData.status) {
        ...
        for (let key in total_details) {
            ...
            const joins = [
                { table: 'expense_management_types as emt', alias: 'emt', condition: ['expense_management.expense_type_id', 'emt.id'] }
            ]
            ...
            let creditexpense = await indexRepository.sum('expense_management', creditCondition, 'due_amount')
            let debitexpense = await indexRepository.sum('expense_management', debitCondition, 'due_amount')
            ...
            if (creditExpensesManagement.status) {
                ...
                for (let keys in creditExpensesManagement.data) {
                    ...
                    credit_expense_object = {
                        ...
                        has_goal_amount: expense.has_goal_amount,
                        earlier_amount_paid: total_deductions.status ? total_deductions.data : 0,
                        amount_paid: expense.due_amount > expense.amount ? expense.amount : expense.due_amount,
            ...
            let total_amt = item.total_amount ? parseFloat(item.total_amount) : 0;
            ...
            } else if (item.visa_type_name == 'H1B' || item.visa_type_name == 'H-1B') {
            ...
            if (item.timesheet_approval_pending) {
                enableSubmit = false;
            }
            ...
            let temp = total_amt - autoCalculatedAmountPaid;

            if (item.current_available_balance) {
                availableBalanceAtThisPay = item.current_available_balance;
            } else {
                availableBalanceAtThisPay = temp;
            }
            ...
            listingObject = {
                ...
                amount_paid: autoCalculatedAmountPaid,
                net_amount: (autoCalculatedAmountPaid + reimbusment_total_amount) - (debitexpense_total_amount),
                debit_expense_available: debitexpense.status ? debitexpense.data : 0,
                credit_expense_available: creditexpense.status ? creditexpense.data : 0,
                period: payrollPeriod,
                balance_amount: item.employment_type_id == 1 ? 0 : (item.balance_amount ? item.balance_amount : 0),
        ...
        return { status: true, data: responseData, pagination_data: pagination_details, payrollPeriod, ot_hours_exists: othoursExistsInPayroll, enable_submit: enableSubmit };
    } else {
        return payrollData;
    }
}
...
const balanceSheetIndexCard = async (req, condition) => {
    ...
    var commonData = {
        ...
        total_billed_amount: total_billed_amount?.[0].sum || 0,
        total_balance_amount: total_balance_amount?.[0].sum || 0,
    }
    ...
    if (financialSummary == 'payroll_summary') {
        response = {
            ...
            data: {
                ...commonData,
                total_paid_amount: total_paid_amount?.[0].sum || 0
            }
        }
    }
    else if (financialSummary === 'expense_summary') {
        if (req?.expense_transaction_type && condition.from_date && condition.to_date) {
            whereClause += ` AND expense_transaction_type = ${req.expense_transaction_type} AND raised_date BETWEEN '${condition.from_date}' AND '${condition.to_date}' AND status = 'Processed'`;
            total_expense_amount = await indexRepository.rawQuery(`${totalExpenses} ${whereClause} `);
        } else if (req?.expense_transaction_type) {
            whereClause += ` AND expense_transaction_type = ${req.expense_transaction_type} AND status = 'Processed' `;
            total_expense_amount = await indexRepository.rawQuery(`${totalExpenses} ${whereClause} `);
        }
        response = {
            status: true,
            data: {
                ...commonData,
                total_expense_amount: total_expense_amount?.[0]?.sum || 0,
            }
        };
    } else {
...
const index = async (condition, page, limit) => {
     ...
     * Fetching the payment details
     */
    const fields = ['payroll_payment_details.*', 'pc.from_date', 'pc.status', 'pc.to_date', 'pc.check_date', 'emp.display_name as employee_name', 'emp.standard_pay_amount', 'emp.id as employee_id', 'employee_categories.name as employee_categorie_name', 'emp.status', 'emp.sub_status', 'emp.employment_type_id'];
    const payrollJoins = [
        { table: 'employee as emp', alias: 'emp', condition: ['emp.id', 'payroll_payment_details.employee_id'] },
        { table: 'employee_categories', alias: '', condition: ['emp.employee_category_id', 'employee_categories.id'] },
    ...
    if (payrollData.status) {
        ...
        for (let key in total_details) {
            ...
            listingObject = {
                ...
                balance_amount: item.balance_amount ? item.balance_amount : 0,
                placement_count: placementCount ? Number(placementCount.count) : 0,
    ...
    return { status: true, data: finallistingObject, pagination_data: final_pagination_data, ot_hours_exists: othoursExistsInPayroll };  // expenseData: empExpense,
};
...
module.exports = { index, update, getInvoiceStatusForPayrollHours, payrollFinalize, balanceSheetIndexCard, generatePayroll, payrollPaymentInfo1, payrollPaymentInfo, calculatePayRates, getSinglePlacementTotalHours, paymentsListing, payrollRun, submitPayroll, finalizeListing, finalizedEmployee, payrollSkip }