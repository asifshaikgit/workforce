...
const { fetchAndMoveDocument, destroyDocument } = require('./globalHelper')
 ...
 * @return Json
 */
const commonDocumentStore = async (db , tablename, destFolder, new_document_id, document_id, requestFileName= null) => {
    ...
    const docData = await indexRepository.find(tablename, ['document_name as name', 'document_url', 'id', 'document_status'], { id: document_id}, 0)
    ...
    await transactionRepository.destroy(db, 'temp_upload_documents', {id: new_document_id  }); // Destroy the temporary stored document
  
    return {status: true}
  };

module.exports =  commonDocumentStore ;
